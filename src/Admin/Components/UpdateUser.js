...
import { getAllUsers, getSingleUser, register, updateUser } from '../../slices/userSlice';
...
export default function UpdateUser({id, open, setOpen }) {
    ...
    const { isUpdated, isLoading, data } = useSelector((state) => state.userCustom)

    const handleSubmit = (e) => {
        e.preventDefault();
        dispatch(updateUser({id, email, newPassword,oldPassword })).then(() => {
            setOpen(false)
            setFlag(true)
        }).then(()=>{
