...
import { deleteLead, deleteSeller, getAllLeads, getAllSeller } from '../slices/leadSlice';
import { useDispatch, useSelector } from 'react-redux';
import { TrashIcon, PencilIcon } from '@heroicons/react/24/solid';
import { IconButton, Tooltip, select } from '@material-tailwind/react';
import VisibilityIcon from '@mui/icons-material/Visibility';
import AddSeller from "../Employee/Components/AddSeller"
...
const AdminSellerPage = () => {
  ...
  const { leads,sellerData } = useSelector((state) => state.leadCustom)
  ...
  const filteredSellers = sellerData && sellerData.filter((seller) =>
          ...
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr className="h-16 w-full text-sm leading-none text-gray-800">
                ...
                <th className="font-normal text-left pl-16">Creator</th>
            ...
            <tbody className="w-full">
              {paginatedSellers?.map((seller, id) => (
                ...
                >
                  ...
                  <td className="pl-12 capitalize">
  <select
    className='capitalize p-1 px-2 border border-gray outline-none'
    onChange={(e) => {
      const selectedProductIndex = e.target.selectedIndex;
      const selectedProduct = seller?.madeProducts[selectedProductIndex];
      if (selectedProduct && selectedProduct.productLink) {
        if (selectedProduct.productLink.startsWith('http')) {
          window.open(selectedProduct.productLink, '_blank');
        } else {
          window.location.href = selectedProduct.productLink;
        }
      }
    }}
  >
    {seller?.madeProducts && seller.madeProducts.map((item, id) => (
      <option key={id} className='capitalize'>
        {item.productName}, {item.price}₹
                  ...
                  <td className="pl-16">
                    <p className="text-sm mb-2  leading-3 text-gray-600">
                      {seller.creator} 
                    </p>
